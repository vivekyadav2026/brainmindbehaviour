...
function sendAppointmentEmail($app) {
    ...
    try {
        ...
        // Add recipients
        $mail->addAddress($CLINIC_EMAIL); // Admin
        if (!empty($app['patient_email'])) {
            $mail->addAddress($app['patient_email']); // Patient
        }
        if (!empty($app['patient_email'])) {
        ...
        <body>
            <div class='container'>
                ...
                <div class='content'>
                    ...
                    <p>Your online appointment has been successfully booked and confirmed. Below are the details:</p>
        ...
        $mail->isHTML(true);
        $mail->Subject = "Appointment Confirmed (Online): " . $app['patient_name'];
        $mail->Body    = $message;
